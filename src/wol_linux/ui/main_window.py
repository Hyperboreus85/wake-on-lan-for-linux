...
class MainWindow(Adw.ApplicationWindow):
    ...
    def _show_preferences(self, *_args: object) -> None:
        ...
        export_button.connect("clicked", lambda *_: self._choose_backup_mode(dialog))
    ...
    def _choose_backup_mode(self, parent: Gtk.Window) -> None:
        dialog = Adw.MessageDialog(
            transient_for=parent,
            heading=_("Come vuoi esportare il backup?"),
            body=_(
                "Il JSON libero è leggibile e modificabile. Il backup cifrato protegge "
                "indirizzi, dettagli e note con una password."
            ),
        )
        dialog.add_response("cancel", _("Annulla"))
        dialog.add_response("json", _("JSON libero"))
        dialog.add_response("encrypted", _("Backup cifrato"))
        dialog.set_default_response("encrypted")
        dialog.set_close_response("cancel")
        dialog.set_response_appearance("encrypted", Adw.ResponseAppearance.SUGGESTED)

        def handle_response(current_dialog: Adw.MessageDialog, response: str) -> None:
            current_dialog.destroy()
            if response == "json":
                self._choose_backup_destination(parent, encrypted=False)
            elif response == "encrypted":
                self._choose_encrypted_backup_password(parent)

        dialog.connect("response", handle_response)
        dialog.present()

    def _choose_encrypted_backup_password(self, parent: Gtk.Window) -> None:
        dialog = Gtk.Dialog(
            title=_("Password backup cifrato"),
            transient_for=parent,
            modal=True,
        )
        dialog.set_default_size(420, 260)
        dialog.add_button(_("Annulla"), Gtk.ResponseType.CANCEL)
        continue_button = dialog.add_button(_("Continua"), Gtk.ResponseType.ACCEPT)
        ...
        content.append(
            Gtk.Label(
                label=_(
                    "La password non viene salvata. Senza di essa il backup non può "
                    "essere recuperato."
                ),
                wrap=True,
                xalign=0,
        ...
        confirm_entry = Gtk.PasswordEntry(
            placeholder_text=_("Conferma password"),
            show_peek_icon=True,
        )
        ...
        def handle_response(current_dialog: Gtk.Dialog, response: int) -> None:
            ...
            if not password:
                self._show_error(
                    _("Esportazione non riuscita"),
                    _("La password del backup non può essere vuota"),
                )
                return
            if password != confirm_entry.get_text():
                self._show_error(_("Esportazione non riuscita"), _("Le password non coincidono"))
                return
            current_dialog.destroy()
            self._choose_backup_destination(parent, encrypted=True, password=password)
    ...
    def _choose_backup_destination(
        ...
        chooser = Gtk.FileChooserNative(
            title=_("Esporta backup cifrato") if encrypted else _("Esporta backup completo"),
            transient_for=parent,
        ...
        chooser.set_current_name(
            "wake-on-lan-for-linux-backup.wolbackup"
            if encrypted
            else "wake-on-lan-for-linux-backup.json"
        )
        file_filter = Gtk.FileFilter(
            name=_("Backup cifrato (.wolbackup)") if encrypted else _("Backup JSON (.json)")
        )
        file_filter.add_pattern("*.wolbackup" if encrypted else "*.json")
        chooser.add_filter(file_filter)
        ...
        def handle_response(current_chooser: Gtk.FileChooserNative, response: int) -> None:
            ...
            try:
                ...
                if encrypted:
                    ...
                    export_encrypted_backup(path, self.settings, computers, password)
                else:
                    export_backup(path, self.settings, computers)
            except (OSError, ValueError) as exc:
            ...
            self.toasts.add_toast(
                Adw.Toast(
                    title=(
                        _("Backup cifrato esportato")
                        if encrypted
                        else _("Backup completo esportato")
                    )
                )
            )
    ...
    def _choose_backup_file(self, parent: Gtk.Window) -> None:
        ...
        file_filter = Gtk.FileFilter(name=_("Backup Wake on LAN for Linux (.json o .wolbackup)"))
        file_filter.add_pattern("*.json")
        file_filter.add_pattern("*.wolbackup")
        chooser.add_filter(file_filter)
        ...
        def handle_response(current_chooser: Gtk.FileChooserNative, response: int) -> None:
            ...
            except BackupPasswordRequired:
                self._choose_import_backup_password(parent, path)
                return
    ...
    def _choose_import_backup_password(self, parent: Gtk.Window, path: str) -> None:
        dialog = Gtk.Dialog(
            title=_("Password backup cifrato"),
            transient_for=parent,
            modal=True,
        ...
        dialog.add_button(_("Annulla"), Gtk.ResponseType.CANCEL)
        import_button = dialog.add_button(_("Importa"), Gtk.ResponseType.ACCEPT)
        ...
        content.append(Gtk.Label(label=_("Inserisci la password del backup cifrato."), xalign=0))
        password_entry = Gtk.PasswordEntry(
            placeholder_text=_("Password"),
            show_peek_icon=True,
        )
        ...
        def handle_response(current_dialog: Gtk.Dialog, response: int) -> None:
            if response != Gtk.ResponseType.ACCEPT:
                ...
                return
            try:
                backup = load_backup(path, password_entry.get_text())
            except (OSError, ValueError, sqlite3.DatabaseError) as exc:
                self._show_error(_("Importazione non riuscita"), str(exc))
                return
            current_dialog.destroy()
    ...
    def _restore_backup(self, backup: BackupData) -> None:
        try:
        ...
        self.settings.custom_colors = dict(backup.settings.custom_colors)
        self.settings.font_family = backup.settings.font_family
        self.settings.font_size = backup.settings.font_size
        self.settings.column_widths = dict(backup.settings.column_widths)
        self.settings.save()
        apply_appearance(self.settings)
        self._refresh_computers()
